      ...
      this.couponService.getCoupons(resp.user.uid, false).subscribe(coupons => {
        this.menus = [
          new MoreMenuGroup('계정 관리', [
            new MoreMenuItem('내 쿠폰', () => { this.router.navigate(['/tabs/coupon-list']) }, '', coupons.length, true),
            new MoreMenuItem('간편결제 관리', () => { this.router.navigate(['/tabs/payment-management']) }, 'card-outline'),
            new MoreMenuItem('개인정보 관리', () => alert('서비스 준비중입니다 ^^'), 'person-circle-outline', 0, false, true),
          ]),
          // new MoreMenuGroup('호스트', [
          //   new MoreMenuItem('모임 만들기', () => this.router.navigate(['/tabs/meeting-add']), 'add-outline'),
          //   new MoreMenuItem('주최한 모임 관리', () => this.router.navigate(['/tabs/hosted-meetings']), 'copy-outline'),
          ...
          new MoreMenuGroup('지원', [
            ...
            new MoreMenuItem('공지사항', () => alert('서비스 준비중입니다 ^^'), '', 0, false, true),
            new MoreMenuItem('자주 묻는 질문', () => alert('서비스 준비중입니다 ^^'), '', 0, false, true)
          ]),
          new MoreMenuGroup('노는법 소개', [
